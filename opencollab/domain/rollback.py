...
EffectKind = Literal["child_result", "message", "tool_result", "spawn", "turn_result"]
...
class EnvironmentSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not name or "=" in name or "\0" in name or "\0" in value for name, value in self.entries):
            raise ValueError("environment entries contain an invalid name or NUL byte")
...
class RollbackPlan:
    ...
    effect_fingerprints: Mapping[str, str] = field(default_factory=dict)

    def digest(self) -> str:
        """Return a deterministic confirmation token for this plan."""
        payload = {
            "target_effect_ids": sorted(self.target_effect_ids),
            "invalidated_effect_ids": sorted(self.invalidated_effect_ids),
            "affected_agent_ids": sorted(self.affected_agent_ids),
            "coordinator_agent_ids": sorted(self.coordinator_agent_ids),
            "checkpoint_by_agent": {
                str(aid): {
                    "id": checkpoint.checkpoint_id,
                    "revision": checkpoint.filesystem_revision,
                    "filesystem_digest": checkpoint.filesystem_digest,
                    "environment_digest": checkpoint.environment.digest(),
                    "identity_digest": hashlib.sha256(checkpoint.workspace_identity.encode()).hexdigest(),
                    "frontier": sorted(checkpoint.causal_frontier),
                    "owner": checkpoint.owner_aid,
                    "sequence": checkpoint.sequence,
                } if checkpoint is not None else None
                for aid, checkpoint in sorted(self.checkpoint_by_agent.items())
            },
            "effect_fingerprints": dict(sorted(self.effect_fingerprints.items())),
        }
...
def select_checkpoint(
    checkpoints: Mapping[int, Iterable[ScopeCheckpoint]],
    aid: int,
    invalidated_effect_ids: set[str],
    historical_invalidated_effect_ids: set[str] | None = None,
) -> ScopeCheckpoint | None:
    """Select the newest checkpoint not causally contaminated by rollback."""
    unavailable = invalidated_effect_ids | (historical_invalidated_effect_ids or set())
    candidates = [
        checkpoint
        for checkpoint in checkpoints.get(aid, ())
        if not checkpoint.causal_frontier.intersection(unavailable)
    ]
